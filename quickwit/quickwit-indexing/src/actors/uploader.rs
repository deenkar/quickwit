        ...
        spawn_named_task(
            async move {
                ...
                for packaged_split in batch.splits.iter() {
                    ...
                    let split_streamer = match SplitPayloadBuilder::get_split_payload_with_recovery_metadata(
...
fn create_split_recovery_metadata(
    ...
    SplitRecoveryMetadata {
        ...
        uncompressed_docs_size_in_bytes: split_metadata.uncompressed_docs_size_in_bytes,
        time_range_start: split_metadata
            .time_range
            .as_ref()
            .map(|range| *range.start()),
        time_range_end: split_metadata.time_range.as_ref().map(|range| *range.end()),
        ...
        footer_start: Some(split_metadata.footer_offsets.start),
        footer_end: Some(split_metadata.footer_offsets.end),
    }
}
...
async fn upload_split(
    ...
    let serialized_recovery_metadata = recovery_metadata.serialize();
    let split_streamer = SplitPayloadBuilder::get_split_payload_with_recovery_metadata(
        &packaged_split.split_files,
        &packaged_split.serialized_split_fields,
        Some(&serialized_recovery_metadata),
        &packaged_split.hotcache_bytes,
